export class TokenCleaner {
  ...
  constructor(
    customPatterns: { regex: RegExp; replacement: string }[] = [],
    ...
    this.secretPatterns = [
      ...
      {
        regex:
          /(?<=(["'])(?:api[_-]?key|stripe[_-]?key|api[_-]?secret|access[_-]?token|auth[_-]?token|client[_-]?secret|password|secret|secret[_-]?key|private[_-]?key|jwt[_-]?secret)["']\s*:\s*["'])([^"']{3,})(?=["'])/gi,
      ...
      {
        regex:
          /(api[_-]?key|stripe[_-]?key|api[_-]?secret|access[_-]?token|auth[_-]?token|client[_-]?secret|password|secret|secret[_-]?key|private[_-]?key|jwt[_-]?secret)\s*=\s*(["'])([^"']{3,})\2/gi,
      ...
      {
        regex:
          /(^\s*(?:export\s+)?)(API[_-]?KEY|API[_-]?SECRET|ACCESS[_-]?TOKEN|AUTH[_-]?TOKEN|CLIENT[_-]?SECRET|DB[_-]?PASSWORD|DATABASE[_-]?PASSWORD|AWS_ACCESS_KEY_ID|AWS_SECRET_ACCESS_KEY|GOOGLE_API_KEY|AZURE_CLIENT_SECRET|DATABASE_URL|MONGO_URI|MYSQL_URL|JWT[_-]?SECRET|SECRET[_-]?KEY|PRIVATE[_-]?KEY)\s*=\s*(?:\"[^\"]{3,}\"|'[^']{3,}'|[^\s#\n]{3,})/gm,
      ...
      {
        regex:
          /(?<=\b(?:api[_-]?key|api[_-]?secret|access[_-]?token|auth[_-]?token|client[_-]?secret|password|secret|secret[_-]?key|private[_-]?key|jwt[_-]?secret)\s*:\s*)(?:\"[^\"]{3,}\"|'[^']{3,}'|[^\s#\n]{3,})/gm,
        replacement: '"[REDACTED]"',
      },
      ...customSecretPatterns,
    ];
